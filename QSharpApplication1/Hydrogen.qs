namespace Quantum.QSharpApplication1 {
	...
	function H2Terms(idxHamiltonian : Int) : (Pauli[], Int[]){
		return [
            ([PauliZ], [0]),
            ([PauliZ], [1]),
            ([PauliZ, PauliZ], [0, 1]),
	...
	operation EvolveUnderHamiltonianTerm(
		...
		qubits : Qubit[]) 
		: Unit is Adj + Ctl{
			let (pauliString, idxQubits) = H2Terms(idxTerm);
			let coeff = (H2Coeff(idxBondLength))[idxTerm];
	...
	operation EvolveUnderHamiltonian(
		...
		qubits : Qubit[]) : Unit is Adj + Ctl{
			let trotterOrder = 1;
			let op = EvolveUnderHamiltonianTerm(idxBondLength, _, _,_);
		}
